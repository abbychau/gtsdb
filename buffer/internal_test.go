...
func TestDeleteDataPoints(t *testing.T) {
	...
	if len(result) != 2 {
		t.Fatalf("Expected 2 points after delete, got %d", len(result))
	}
...
func TestDeleteDataPointsDeletesWholeKey(t *testing.T) {
	...
	if _, err := os.Stat(utils.DataDir + "/" + id + ".aof"); !os.IsNotExist(err) {
		t.Fatalf("Expected %s.aof to be deleted", id)
	}
